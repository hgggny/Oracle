/*
    <TCL(Transaction Control Language)>
        ...
        DML(INSERT, UPDATE, DELETE)을 대상으로 진행이 된다. 
...
-- 두 개의 행이 삭제된 시점에 SAVEPOINT 지정 (213, 218 삭제)
SAVEPOINT SP1;
...
-- SAVEPOINT를 지정했던 213, 218번은 복구가 안된다.
ROLLBACK TO SP1;  

ROLLBACK;

-- 모두 돌아온다. (213, 218번도 모두 복구가 된다.)
SELECT * FROM EMP_TEST;

-- DDL(CREATE, ALTER, DROP) 구문 실행 시 변경사항들은 무조건 DB에 반영된다. (COMMIT 된다.)
CREATE TABLE TEST (
    TID NUMBER
);
