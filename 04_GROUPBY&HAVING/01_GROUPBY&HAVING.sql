...
SELECT NVL(DEPT_CODE, '부서 없음'), COUNT(*)||'명'
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE NULLS FIRST;
...
SELECT JOB_CODE AS "직급",
       TO_CHAR(FLOOR(AVG(NVL(SALARY, 0))), 'FML999,999,999') AS "급여 평균"
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE;
...
/*
    <집게 함수>
        그룹별로 산출한 결과 값의 중간 집계를 계산하는 함수
*/

-- EMPLOYEE 테이블에서 각 직급별 급여의 합계를 조회
...
-- EMPLOYEE 테이블에서 부서 코드가 D5인 사원들의 사번, 직원명, 부서코드, 급여 조회 (6명)
...
/*
    <GROUPING SET>
        그룹 별로 처리된 여러 개의 SECLECT 문을 하나로 합친 결과로 원할 때 사용한다.
*/
