...
/*
    3) 컬럼 삭제 (DROP COLUMN)
        ...
        - 자식테이블이 참조하고 있는 부모 테이블은 삭제가 되지 않는다. 
*/
...
ALTER TABLE MEMBER_GRDE DROP COLUMN GRADE_CODE; -- <cannot drop parent key column>

-- CASCADE CONSTRAINTS 옵션을 넣으면 삭제 가능하다. 
-- 실수하는 상황이 오기 때문에 사용할 때는 유의해야 한다. 
ALTER TABLE MEMBER_GRDE DROP COLUMN GRADE_CODE CASCADE CONSTRAINTS;
...
/*
    2. 제약조건 추가 / 삭제 / 이름 변경
        ...
        FOREIGN KEY : ALTER TABLE 테이블명 ADD [CONSTRAINT 제약조건명] FOREIGN KEY(컬럼명) REFERENCE 테이블명[(컬럼명)];
...
-- DEPT_COPY 테이블 DEPT_ID 컬럼에 PK 제약조건 추가
...
/*
    2) 제약조건 삭제
        ...
        나머지   : ALTER TABLE 테이블명 DROP CONSTRATIN 제약조건명;
*/
...
*/
-- DEPT_COPY 테이블에 SYS_C007473 제약조건명을 DEPT_COPY_DEPT_ID_NN으로 변경
ALTER TABLE DEPT_COPY RENAME CONSTRAINT SYS_C007473 TO DEPT_COPY_DEPT_ID_NN;
...
DROP TABLE MEMBER_GRADE CASCADE CONSTRAINT; -- MEMBER 테이블의 FOREIGN KEY가 삭제되며, MEMBER_GRADE 테이블이 삭제된다. 

ROLLBACK; -- DDL 구문에서는 사용할 수 없다. 
